(function () {
  ...
  let API_URL = localStorage.getItem('asc_api_url') || '';
  ...
  window.togglePass = (id, btn) => { const el = document.getElementById(id); if (el.type === 'password') { el.type = 'text'; btn.textContent = '🙈'; } else { el.type = 'password'; btn.textContent = '👁'; } };

  document.addEventListener('DOMContentLoaded', () => { initAuth(); });

  function initAuth() {
    if (TOKEN) { verifyTokenAndLoad(); } else { showAuthScreen(); }
  ...
  async function verifyTokenAndLoad() {
    try { const res = await apiFetch('/api/auth/me'); CURRENT_USER = res.user; showApp(); }
    catch { TOKEN = null; localStorage.removeItem('asc_token'); showAuthScreen(); }
  }

  function showAuthScreen() { document.getElementById('auth-screen').classList.remove('hidden'); document.getElementById('app-screen').classList.add('hidden'); }
  ...
  async function doLogin() {
    ...
    try {
      ...
      TOKEN = data.token; CURRENT_USER = data.user; localStorage.setItem('asc_token', TOKEN); showApp();
    } catch { showAuthError('login-error', 'Nao foi possivel conectar ao servidor.'); }
  ...
  function initApp() {
    initNavigation(); initScriptForm(); initCallForm(); initAnalyzeForm(); initSettings(); initInputToggles(); initLogout(); loadDashboard();
    checkServerStatus(); setInterval(checkServerStatus, 30000);
  }

  function initLogout() {
    const doLogout = () => { TOKEN = null; CURRENT_USER = null; localStorage.removeItem('asc_token'); showAuthScreen(); };
    document.getElementById('logout-btn').addEventListener('click', e => { e.preventDefault(); doLogout(); });
    const s = document.getElementById('logout-btn-settings'); if (s) s.addEventListener('click', doLogout);
  }

  function initNavigation() {
    document.querySelectorAll('.nav-item').forEach(item => {
      item.addEventListener('click', e => {
        e.preventDefault(); switchTab(item.dataset.tab);
        document.querySelectorAll('.nav-item').forEach(n => n.classList.remove('active')); item.classList.add('active');
      });
    });
    document.getElementById('refresh-btn').addEventListener('click', () => { const active = document.querySelector('.tab-content.active').id.replace('tab-', ''); loadTab(active); });
  }
  ...
  function loadTab(tab) { const map = { dashboard: loadDashboard, scripts: loadScripts, calls: loadCalls, knowledge: loadKnowledge, settings: loadSettings }; if (map[tab]) map[tab](); }
  ...
  function setupToggle(btnIds, elIds) {
    ...
    btns.forEach((btn, i) => { btn.addEventListener('click', () => { btns.forEach(b => b.classList.remove('active')); els.forEach(e => e.classList.remove('active')); btn.classList.add('active'); if (els[i]) els[i].classList.add('active'); }); });
  }
  ...
  function handleFile(file, onText, isAudio, drop) {
    if (isAudio || file.type.startsWith('audio/')) { drop.querySelector('.file-drop-text').textContent = '🎙️ ' + file.name; drop.querySelector('.file-drop-sub').textContent = (file.size/1024/1024).toFixed(1) + ' MB'; drop._audioFile = file; return; }
    const reader = new FileReader();
    reader.onload = e => { if (onText) onText(e.target.result); drop.querySelector('.file-drop-text').textContent = '✅ ' + file.name; toast('Arquivo carregado!', 'success'); };
  ...
  async function loadDashboard() {
    try {
      ...
      document.getElementById('knowledge-summary').textContent = knowledge.summary || 'Nenhum conhecimento. Adicione scripts e calls.';
    } catch { document.getElementById('knowledge-summary').textContent = 'Erro ao carregar. Verifique o backend.'; }
    const btn = document.getElementById('rebuild-btn'); if (btn) btn.onclick = rebuildKnowledge;
  }
  ...
  function initScriptForm() {
    document.getElementById('script-form').addEventListener('submit', async e => {
      ...
      try {
        ...
        document.getElementById('script-form').reset(); loadScripts(); loadDashboard();
  ...
  async function loadScripts() {
    try {
      ...
      const countEl = document.getElementById('scripts-count'); if (countEl) countEl.textContent = scripts.length;
  ...
  function initCallForm() {
    document.getElementById('call-form').addEventListener('submit', async e => {
      ...
      try {
        ...
        document.getElementById('call-form').reset(); loadCalls(); loadDashboard();
      } catch (err) { toast('Erro: ' + err.message, 'error'); }
  ...
  async function loadCalls() {
    try {
      ...
      const countEl = document.getElementById('calls-count'); if (countEl) countEl.textContent = calls.length;
  ...
  function initAnalyzeForm() {
    document.getElementById('analyze-btn').addEventListener('click', async () => {
      ...
      setLoading('analyze-btn', true); document.getElementById('analyze-result').classList.add('hidden');
      try {
        const res = await apiJSON('/api/analyze', { transcript: content });
        if (res.success) renderAnalysis(res.analysis); else toast('Erro: ' + (res.error||'desconhecido'), 'error');
      } catch (err) { toast('Erro: ' + err.message, 'error'); }
  ...
  async function loadKnowledge() {
    try { const data = await apiFetch('/api/knowledge'); const el = document.getElementById('knowledge-summary-2'); if (el) el.textContent = data.summary || 'Nenhum resumo. Adicione scripts e calls.'; } catch {}
    const btn = document.getElementById('rebuild-btn-2'); if (btn) btn.onclick = rebuildKnowledge;
  }

  async function rebuildKnowledge() { toast('Reprocessando...', 'success'); await loadDashboard(); await loadKnowledge(); toast('Base atualizada!', 'success'); }

  function loadSettings() {
    document.getElementById('backend-url').value = API_URL || window.location.origin;
    document.getElementById('ws-url').value = localStorage.getItem('asc_ws_url') || window.location.origin.replace('https:', 'wss:').replace('http:', 'ws:');
  }

  function initSettings() {
    document.getElementById('save-settings-btn').addEventListener('click', () => {
      API_URL = document.getElementById('backend-url').value.trim();
      localStorage.setItem('asc_api_url', API_URL); localStorage.setItem('asc_ws_url', document.getElementById('ws-url').value.trim());
      toast('Salvo!', 'success'); checkServerStatus();
    });
    document.getElementById('test-connection-btn').addEventListener('click', async () => {
      const result = document.getElementById('connection-result');
      try { const data = await apiFetch('/api/health'); result.className = 'connection-result success'; result.textContent = '✅ Conectado! Scripts: ' + data.knowledge?.scripts + ', Calls: ' + data.knowledge?.calls; }
      catch { result.className = 'connection-result error'; result.textContent = '❌ Falha. Backend offline em ' + (API_URL || window.location.origin); }
    });
  }

  async function checkServerStatus() {
    const el = document.getElementById('server-status');
    try { await fetch((API_URL || '') + '/api/health'); el.innerHTML = '<span class="status-dot status-dot--online"></span><span>Backend online</span>'; }
    catch { el.innerHTML = '<span class="status-dot status-dot--offline"></span><span>Backend offline</span>'; }
  }

...
// ── Rota de seed interno — adiciona scripts/calls via JSON diretamente ────────
        ...
        try {
                  ...
                  for (const s of scripts) {
                              const r = await pool.query(
                                            'INSERT INTO scripts (user_id, title, description, content) VALUES ($1,$2,$3,$4) RETURNING id, title',
                                            [req.user.id, s.title, s.description || null, s.content || null]
                                          );
                              results.scripts.push(r.rows[0]);
                  }
                  for (const c of calls) {
                              const r = await pool.query(
                                            'INSERT INTO calls (user_id, title, outcome, notes, content) VALUES ($1,$2,$3,$4,$5) RETURNING id, title',
                                            [req.user.id, c.title, c.outcome || 'venda', c.notes || null, c.content || null]
                                          );

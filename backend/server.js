...
function buildCoachingPrompt(transcript, knowledge, insightsJaEntregues) {
  ...
  PROIBIDO GERAR:
  - Conselhos genericos descolados do que foi dito ("seja empatico", "ouça mais", "faca perguntas abertas")
...
wss.on('connection', async (ws, req) => {
    ...
    if (tokenParam) { const p = verifyToken(tokenParam); if (p) userId = p.id; }
         ...
         const session = {
               ...
               speechCount: 0,
               lastInsightAt: 0,
               minIntervalMs: 18000, // minimo 18s entre analises automaticas — evita ruido em rajada
         ...
         ws.on('message', async (raw) => {
               ...
               switch (msg.type) {
                 ...
                 case 'TRANSCRIPT_CHUNK': {
                           ...
                           session.speechCount++;
                           const now = Date.now();
                           // Analisa automaticamente apos 6 falas e respeitando o intervalo minimo
                           if (session.speechCount >= 6 && (now - session.lastInsightAt) >= session.minIntervalMs && !session.pendingAnalysis) {
                                       ...
                                       session.speechCount = 0; // reset contador apos cada analise automatica
...
async function analyzeTranscript(session, force = false) {
    // Auto: analisa os ultimos 3500 chars | Manual (force): analisa os ultimos 2500 chars com modelo mais capaz
  const slice = force
      ? session.fullTranscript.slice(-2500)
        : session.fullTranscript.slice(-3500);
    if (slice.trim().length < 30) return null;

  const model = force ? 'gpt-4o' : 'gpt-4o-mini'; // manual usa modelo mais preciso
  const temperature = force ? 0.15 : 0.2;           // manual ainda mais deterministico
